...
def vote(vote: schemas.Vote, db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if not post: #In case the user tries to vote on a post that doesn't exist
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Non-existent post with id: {vote.post_id}")
